...
export const TransferPage: React.FC = () => {
  ...
  const [tempPassData, setTempPassData] = useState<any>(null);
  ...
  const handleSendOtp = async () => {
    if (!selectedTransfer || !assignmentCourierId) {
      toast.error('Please select a Courier Boy first');
      return;
    }
    setSendingOtp(true);
    try {
      ...
      setOtpSent(true);
      setOtpValue('');
      if (res.data?.data?.otpDevMode) {
        setOtpDevCode(res.data.data.otpDevMode);
        toast.success(`OTP sent! (Dev mode: ${res.data.data.otpDevMode})`, { duration: 8000 });
      } else {
        toast.success(`OTP sent to Courier Boy's phone!`);
      }
    } catch (err: any) {
  ...
  const handleAssignCourier = async () => {
    ...
    if (!otpSent) {
      toast.error('Please send OTP to Courier Boy first');
      return;
    }
    if (!otpValue.trim()) {
      toast.error('Please enter the OTP received by the Courier Boy');
      return;
    }
    ...
    try {
      ...
      toast.error(err.response?.data?.message || 'Failed to assign courier — check OTP');
    } finally {
            ...
            <div className="grid grid-cols-2 gap-4 text-xs bg-slate-950/40 p-3 rounded-lg border border-slate-800/40">
              ...
              <div className="mt-2 col-span-2 border-t border-slate-850 pt-2">
                ...
                {canAssignCourierForTransfer(selectedTransfer) && (
                  <div className="mt-3 rounded-lg border border-amber-500/20 bg-amber-500/5 p-3 space-y-3">
                    <label className="text-[10px] font-bold uppercase tracking-wider text-amber-300 flex items-center space-x-1.5">
                      <UserCheck className="h-3.5 w-3.5" />
                      <span>STEP 3 — Branch Manager: Assign Courier via OTP</span>
                    </label>

                    <div className="flex flex-col sm:flex-row gap-2">
                      <select
                        value={assignmentCourierId}
                        onChange={(e) => { setAssignmentCourierId(e.target.value); setOtpSent(false); setOtpValue(''); setOtpDevCode(''); }}
                      ...
                      >
                        <option value="">
                          {assignmentCouriersLoading ? 'Loading couriers...' : assignmentCouriers.length === 0 ? 'No courier boys available' : 'Select courier boy'}
                        </option>
                        {assignmentCouriers.map((c: any) => (
                          <option key={c._id} value={c._id}>
                            {`${c.firstName} ${c.lastName} (${c.employeeId})${c.phone ? ' — ' + c.phone : ''}`}
                          </option>
                        ))}
                      </select>
                      <Button type="button" size="sm" onClick={handleSendOtp}
                        disabled={sendingOtp || !assignmentCourierId}
                        className="h-9 shrink-0 bg-blue-600 hover:bg-blue-500 text-white font-bold">
                    ...
                    {otpSent && (
                      <div className="space-y-2 bg-blue-500/10 border border-blue-500/20 rounded-lg p-3">
                        <p className="text-xs text-blue-300 font-semibold">
                          OTP sent to Courier Boy phone!
                          {otpDevCode && <span className="ml-2 text-yellow-300">[Dev OTP: <strong>{otpDevCode}</strong>]</span>}
                        </p>
                        <p className="text-[11px] text-slate-400">Ask courier boy for the OTP received on their phone:</p>
                        <div className="flex gap-2">
                          <Input value={otpValue} onChange={(e) => setOtpValue(e.target.value)}
                            placeholder="Enter 4-digit OTP" maxLength={6}
                            className="font-mono text-lg tracking-widest text-center bg-slate-950 border-blue-500/40 text-blue-300 font-bold" />
                          <Button type="button" size="sm" onClick={handleAssignCourier}
                            disabled={assigningCourier || !otpValue.trim()}
                            className="h-10 shrink-0 bg-emerald-600 hover:bg-emerald-500 text-white font-bold">
                            {assigningCourier ? 'Verifying...' : 'Verify & Assign'}
                          </Button>
                        </div>
                      </div>
                    )}

                    {!otpSent && assignmentCourierId && (
                      <p className="text-[10px] text-slate-500">Select courier boy and click Send OTP — OTP will be sent to their phone.</p>
                    )}
                  </div>
                )}
              </div>
