...
export const Sidebar: React.FC = () => {
  ...
  const navItems = [
    { label: 'Dashboard', path: ROUTES.DASHBOARD, icon: LayoutDashboard, roles: ['super_admin', 'branch_admin', 'store_manager', 'warehouse_manager', 'authorized_person', 'security_guard', 'staff'] },
    ...
    { label: 'Products', path: ROUTES.PRODUCTS, icon: Package, roles: ['super_admin', 'branch_admin', 'store_manager', 'warehouse_manager'] },
  ...
  return (
    ...
    >
      <div>
        <div className="h-16 flex items-center px-4 gradient-border-b justify-between">
          <div className="flex items-center space-x-2 overflow-hidden">
            <div className="h-8 w-8 rounded-lg bg-gradient-to-br from-indigo-600 to-violet-600 flex items-center justify-center font-bold text-white shrink-0 shadow-md shadow-indigo-600/25">
              AE
            </div>
            {sidebarOpen && (
            ...
            <NavLink
              ...
              className={({ isActive }) =>
                cn(
                  ...
                  isActive
                    ? "bg-indigo-600/10 text-indigo-500 dark:text-indigo-400 nav-active-indicator rounded-l-none"
      ...
      <div className="p-3 border-t border-border space-y-2">
        {sidebarOpen && user && (
          <div className="flex items-center space-x-3 p-2 bg-muted/40 rounded-lg border border-border glass-card-premium">
            <div className="h-9 w-9 rounded-full bg-indigo-600/20 text-indigo-400 font-bold flex items-center justify-center border border-indigo-500/30 uppercase ring-2 ring-indigo-500/30">
              {user.firstName[0]}
            </div>
        ...
        <div className="flex items-center justify-between">
          <Button variant="ghost" size="icon" onClick={logout} className="text-slate-400 hover:text-red-400 hover:bg-red-950/20 rounded-lg cursor-pointer" title="Log Out">
            <LogOut className="h-5 w-5" />
          </Button>
          <Button variant="ghost" size="icon" onClick={toggleSidebar} className="text-slate-400 hover:text-slate-100 hover:bg-slate-800/60 rounded-lg cursor-pointer hidden md:flex" title={sidebarOpen ? "Collapse Sidebar" : "Expand Sidebar"}>
